DELIMITER ;;
CREATE PROCEDURE `StoredUpdateUserWishlist` (IN `MediaType` varchar(50), IN `MediaId` varchar(100), IN `UserId` int)
BEGIN
...
IF (SELECT IF (
 ...
 THEN
   ...
   ELSEIF (SELECT IF (
         EXISTS(
            SELECT * FROM USERMEDIA WHERE IDMEDIA = MediaId AND MEDIATYPE = MediaType AND IDUSER = UserId AND WISHLIST = 0),1,0)) = 1
   THEN
      UPDATE USERMEDIA SET WISHLIST = 1 , COLLECTION = 0 WHERE IDUSER = UserId AND IDMEDIA = MediaId AND MEDIATYPE = MediaType;
   ELSEIF (SELECT IF (
         EXISTS(
            SELECT * FROM USERMEDIA WHERE IDMEDIA = MediaId AND MEDIATYPE = MediaType AND IDUSER = UserId AND WISHLIST = 1),1,0)) = 1
   THEN
