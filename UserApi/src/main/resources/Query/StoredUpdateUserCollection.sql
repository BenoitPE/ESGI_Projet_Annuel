...
IF (SELECT IF (
 ...
 THEN
   IF (SELECT IF (
         EXISTS(
            SELECT * FROM USERMEDIA WHERE IDMEDIA = MediaId AND MEDIATYPE = MediaType AND COLLECTION = 0),1,0)) = 1
   THEN
      UPDATE USERMEDIA SET WISHLIST = 0 , COLLECTION = 1 WHERE IDUSER = UserId AND IDMEDIA = MediaId AND MEDIATYPE = MediaType;
   ELSEIF (SELECT IF (
         EXISTS(
            SELECT * FROM USERMEDIA WHERE IDMEDIA = MediaId AND MEDIATYPE = MediaType AND COLLECTION = 1),1,0)) = 1
   THEN
